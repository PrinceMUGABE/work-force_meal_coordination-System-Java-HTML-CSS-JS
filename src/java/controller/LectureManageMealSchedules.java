...
@WebServlet("/lecturer/schedule/*")
public class LectureManageMealSchedules extends HttpServlet {
...
protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if ("/list".equals(pathInfo)) {
        listUserSchedules(request, response, loggedInUser);
    } else {
        sendErrorResponse(response, "Invalid endpoint", HttpServletResponse.SC_BAD_REQUEST);
    }
}

private void listUserSchedules(HttpServletRequest request, HttpServletResponse response, User loggedInUser)
        throws IOException {
    try {
        response.setContentType("application/json");
        response.setCharacterEncoding("UTF-8");
        
        List<MealSchedule> schedules = mealScheduleDAO.getSchedulesByUser(loggedInUser);
        
        if (schedules == null || schedules.isEmpty()) {
            System.out.println("No schedules found for user: " + loggedInUser.getEmail());
            sendErrorResponse(response, "No schedules found", HttpServletResponse.SC_NO_CONTENT);
            return;
        }
        
        System.out.println("Total Schedules Retrieved for user " + loggedInUser.getEmail() + ": " + schedules.size());
